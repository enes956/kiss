...
## Dizin Yapısı
- `main.js`: Electron ana proses giriş noktası; yönetici yükseltmesi, güncelleme denetimi, oturum açma IPC köprüleri ve pencere oluşturma burada yapılır. Güncelleme tarafı artık şifresiz `app.asar` paketini doğrudan indirip boyut + hash doğrulamasıyla kullanıcı verisi dizinine `app_new.asar` olarak yazar; eski `app_new.bin` çıktıları uyumluluk için desteklenir ve pending dosyası varsa development modunda bile patch uygulanır. `asar.listPackage` ile offline doğrulama yapıldığı için bozuk paketler pending bırakılmadan reddedilir. Patch başarıyla uygulandığında ana süreç kendini yeniden başlatır, böylece yeni `app.asar` aynı oturumda aktifleşir.
...
- `build.js`: Obfuscation, electron-builder paketleme ve şifresiz güncelleme çıktılarının üretildiği Node betiği.
...
- **Güncelleme Sistemi:**
  ...
  - Eksik kalan IPC köprüsü tamamlandı: UPDATE_* kanalları renderer’dan gelen `update:check` / `update:start` / `update:done` isteklerini dinler, durum/ilerleme ve hata mesajlarını renderer’a geri yollar. Güncel sürümde otomatik login geçişi için `update:done` ana süreç tarafından yeniden yayınlanır.
  - Güncelleme sürecinin her adımı loglanıyor: indirilen baytlar ve Content-Length, ASAR boyutu, hash doğrulaması ve pending flag yazımı konsola düşüyor. Patch aşaması da bulunan ASAR boyutunu ve doğrulama sonucunu raporluyor; bozuk dosya tespit edilirse otomatik olarak siliniyor.
  - `applyStartupPatch` `app.isPackaged === false` iken patch denemesini tamamen atlar; development modunda Electron’un kendi `resources/app.asar` dosyası değiştirilmediği için “Invalid package .../app_new.asar” hataları kesilir. Pending dosyası dev modda yalnızca bilgilendirme amaçlı loglanır.
...
- Oluşan `app.asar`, `dist/dist-build/win-unpacked/resources` altından alınır ve şifresiz biçimde `dist-update/app.asar` olarak kopyalanır.
- Aynı betik, düz `version.json` üretir ve hem `dist-update/` hem `dist/` içine bırakır. Uzak güncelleyiciye yüklenmesi gereken dosyalar: `dist-update/app.asar` ve `dist-update/version.json`.
...
1. `node build.js` → `dist-update/app.asar` ve `version.json` üretimi.
2. Dosyaları güncelleme sunucusuna (`updater.bekapvc.com`) yükleyin.
3. Uygulama açıldığında `main.js` sürümü kontrol eder, yeni ASAR’ı şifresiz indirip doğrular ve bir sonraki açılış için `update_pending.json` oluşturur.
